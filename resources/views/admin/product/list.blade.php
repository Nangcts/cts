                    ...
                    <table class="display table table-bordered table-striped table-responsive" id="product-list">
                        ...
                        <tbody>
                            ...
                            <tr class="" id="tr_{{$item->id}}">
                                <td>{!! $item->id !!}</td>
                                <td class="center"><img style="width:45px; height: 45px;"
                                        src="{{ asset('upload/filemanager/product/thumbs/' . $item->image) }}" alt="">
                                </td>
                                <td class="td-name"><a href="{{ route('allRoute', $item->slug) }}">{!! $item->name
                                        !!}</a></td>
                                ...
                                <td>
                                    ...
                                    <ul>
                                        ...
                                        <li><a href="{{ route('allRoute', $category_item->slug) }}"><i
                                                    class="fa fa-arrow-right"
                                                    style="margin-right: 7px; font-size: 10px"></i>{{ $category_item->name }}</a>
                                        </li>
                                        @endforeach
                                    </ul>
                                </td>
                                ...
                                <td class="center">
                                    <a title="Sửa sản phẩm" data-toggle="modal" class="btn btn-primary btn-xs"
                                        href="{!! route('admin.product.getEdit',$item->id) !!}"><i
                                            class="fa fa-pencil"></i></a>
                                    <a title="Xóa sản phẩm" class="btn btn-danger btn-xs"
                                        href="{!! route('admin.product.delete', $item->id) !!}"
                                        onclick="return confirmdelete('Bạn có chắc sẽ xóa Sản phẩm này?')"><i
