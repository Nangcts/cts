...
<section class="wrapper">
  ...
  <section class="panel">
    <div class="col-lg-12 header-page-admin">
      {!! Breadcrumbs::render('admin-product-offer') !!}
      <header class="panel-heading">
        <strong>Quản lý sản phẩm giới thiệu</strong>
      </header>
    </div>
            ...
            <div class="hot-product-order">
              <ul class="reorder_ul reorder-photos-list">
                ...
                <li id="{{$item->id}}" class="ui-sortable-handle col-lg-3 col-md-3 col-sm-3 col-xs-4" style="float: left;width: 100%">
                  ...
                  <div class="more-link">
                    <a style="padding: 5px 12px; border: 1px solid #ccc;" title="Gỡ sản phẩm khỏi danh sách" class="btn btn-xs" href = "{!! route('removeOfferProduct', $item->id) !!}" onclick="return confirmdelete('Bạn có chắc sẽ loại sản phẩm này ra khỏi danh sách?')"><i class="fa fa-trash" style="color: red;"></i></a>
                  </div>
                </li>
                @endforeach
                @endif
    ...
    $('.reorder_link').on('click',function(){
      ...
      $("#save_reorder").click(function( e ){
        if( !$("#save_reorder i").length ){
          ...
          $.ajax({
            type: "POST",
            url: "/admin/product/reorder-offer-products",
